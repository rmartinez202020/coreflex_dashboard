...
export default function GraphicDisplaySettingsModal({
  ...
  // ✅ local form state (initialized safely)
  const [title, setTitle] = useState("Graphic Display");
  const [timeUnit, setTimeUnit] = useState("seconds");
  const [windowSize, setWindowSize] = useState(60);
  const [sampleMs, setSampleMs] = useState(1000);

  // ✅ IMPORTANT: when user double-clicks a different graphic, load its values
  useEffect(() => {
    if (!tank) return;
  ...
  return (
    ...
    >
      ...
      >
        {/* ✅ more padding so title isn't glued to the border */}
        <div
          style={{
            padding: "16px 18px",
