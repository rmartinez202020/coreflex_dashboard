// src/components/Sidebarleftwirelesstankmodal2.jsx
...
function TelemetryCard({ icon, label, value, accent = "#0f172a", bg = "#f8fafc" }) {
  return (
    <div
      style={{
        display: "grid",
    ...
    >
      ...
      <div>
        <div style={{ fontSize: 11, fontWeight: 900, color: "#0f172a", marginBottom: 4 }}>
          {label}
        </div>
...
export default function Sidebarleftwirelesstankmodal2({
  open = true,
  tank,
  onSave,
  onClose,
  ...
  const [title, setTitle] = useState(props.name || "Tank#2");
  ...
  const onDragMove = (e) => {
    ...
    setPos({
      left: Math.min(
        Math.max(margin, dragRef.current.startLeft + dx),
        Math.max(margin, w - margin - 260)
      ),
      top: Math.min(
        Math.max(margin, dragRef.current.startTop + dy),
        ...
        <div style={{ padding: 16, background: "#f8fafc" }}>
          ...
          >
            ...
            >
              ...
              >
                <Sidebarleftwirelesstank2 size={300} strokeColor="#0f172a" showTelemetry={false} />
              </div>
