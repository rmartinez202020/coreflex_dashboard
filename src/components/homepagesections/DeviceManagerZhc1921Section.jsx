...
export default function DeviceManagerZhc1921Section({
  ...
  // Search by Device ID or claimed user/email.
  ...
  async function addZhc1921Device() {
    ...
    try {
      ...
      // ✅ ADD NEW STEP (do not modify old route)
  ...
  const filteredRows = React.useMemo(() => {
    ...
    return effectiveRows.filter((r) => {
      const deviceId = String(r?.deviceId || "").toLowerCase();
      const claimedUser = String(r?.ownedBy || "").toLowerCase();
      return deviceId.includes(q) || claimedUser.includes(q);
    });
  }, [effectiveRows, deviceSearch]);

  const renderZhc1921Table = () => (

    <div className="rounded-xl border border-slate-200 bg-white overflow-hidden w-full max-w-full">

      <div className="w-full max-h-[520px] overflow-auto overscroll-contain">
            ...
            {loading ? (
              ...
              <tr>
                ...
                >
                  ...
                  {deviceSearch.trim() ? "No matching devices found." : "No devices found."}
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="Search by Device ID or claimed user..."
                className="w-full rounded-lg border border-slate-200 py-2 pl-10 pr-10 text-sm outline-none focus:ring-2 focus:ring-slate-300"
              />
            ...
            <div className="mt-2 text-xs text-slate-500">
              Search the table by DEVICE ID or the user/email that claimed the device.
            </div>

            {!!deviceSearch.trim() && (
              <div className="mt-2 text-xs font-medium text-slate-700">
                Showing {filteredRows.length} of {effectiveRows.length} devices
              </div>
            )}
          </div>
        </div>
